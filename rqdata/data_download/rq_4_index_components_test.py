    ...
    with tqdm(total=len(index_code), desc="Downloading & Processing Index Components", ncols=100) as pbar:
        for code in index_code:
            try:
                ...
                # logger.info(f"index:{code} | start:{start_date.date()} | end:{end_date.date()} saved")

                # --- 转换为 csi 文件 ---
                csi_filename = f"{data_path}/basic_data/index_componens/csi_{code}.csv"
                # try:
                #     index_componens_csi = index_componens_csi_file(csi_filename)
                #     if index_componens_csi.empty:
                #         i_start_date = pd.to_datetime(index_componens_file['tradedate'].min())
                #         is_firstday = True
                #         last_trade_date = ""
                #     else:
                #         last_trade_date = pd.to_datetime(index_componens_csi['end_date'].max())
                #         i_start_date = last_trade_date + pd.Timedelta(days=1)
                #         df_yesterday = index_componens_csi[index_componens_csi['end_date'] == last_trade_date]
                
                #         index_componens_csi = index_componens_csi[index_componens_csi['end_date'] != last_trade_date]
                ...
                # print(index_componens_csi)
                
                success_count += 1
                # logger.success(f"index:{code} | Completed successfully")
            except Exception as e:
                fail_count += 1
