...
dirname = "/home/a/notobook/zxf/code"
...
class Backtest:
    ...
    def read_pred(self):
        ...
        try:
            ...
            df_pred = df_pred[["datetime", "instrument", "score"]]
            # print(df_pred)
    ...
    def predition(self,trade_date,df_pred_today,df_close_today,positive_ratio_today,pred_median_today):
        ...
        if not position_details_today.empty:
            ...
            # print(df_pred_today_top)
            # print(intending_buy)
            # print(set(intending_buy['instrument']))

            inst_pred = (
                df_pred_today_top['instrument']
                if 'instrument' in df_pred_today_top.columns
                else []
            )
            inst_buy = (
                intending_buy['instrument']
                if 'instrument' in intending_buy.columns
                else []
            )
            combined_set = set(inst_pred) | set(inst_buy)
    ...
    def run(self):
        ...
        # float_cols = self._account_details.select_dtypes(include=['float64', 'float32']).columns
        # formatters = {col: '{:.3f}'.format for col in float_cols}
        
        self._position_details.to_csv(f'{output_path}/position_details_{self._top_k}_{self._n_drop}_{self._hold_p}.csv', index=False, date_format='%Y-%m-%d')
        self._account_details.to_csv(f'{output_path}/account_details_{self._top_k}_{self._n_drop}_{self._hold_p}.csv', index=False, date_format='%Y-%m-%d')
