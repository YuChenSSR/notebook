...
def get_stock_list(stock_list_path):
    ...
    # xxx = stock_list['exchange'].drop_duplicates()
    # # print(stock_list.tail(5).to_string(max_cols=None))
    # XSHE = stock_list[stock_list['exchange']=="XSHE"]
    # XSHG = stock_list[stock_list['exchange']=="XSHG"]
    # BJSE = stock_list[stock_list['exchange']=="BJSE"]
    # print(f"XSHE:{len(XSHE)}/XSHG:{len(XSHG)}/BJSE:{len(BJSE)}")
...
def get_stock_factor_rq(code, start_date, end_date):
    try:
        stock_data = rqdatac.get_factor(code, factor, start_date=start_date, end_date=end_date, universe=None, expect_df=True)
        time.sleep(0.1)
...
def stock_factor_run(
    ...
    logger.info(f"Stock Factor Daily Start Downloading[market:{market}]...")
    ...
    if code is None:
        success_count = 0
        fail_count = 0
        with tqdm(total=len(stock_list), desc="Stock Factor Daily Downloading", ncols=100) as pbar:
                ...
                try:
                    ...
                    try:
                        ...
                        if stock_factor_file.empty:
                            stock_factor = get_stock_factor_rq(code=code, start_date=s_start_date, end_date=s_end_date)
                            
                            if stock_factor is None or stock_factor.empty:
                                fail_count += 1
                                logger.error(f"stock:{code}|start_date:{s_start_date.date()}|end_date:{s_end_date.date()}| Data null")
                                continue
        
                        else:
                            if stock_factor_file['date'].iloc[-1] < s_end_date:
                                start_date_new = stock_factor_file['date'].iloc[-1] + pd.Timedelta(days=1)
                                stock_factor_rq = get_stock_factor_rq(code, start_date_new, s_end_date)
        
                            
                                if stock_factor_rq is None or stock_factor_rq.empty:
                                    fail_count += 1
                                    logger.error(f"stock:{code}|start_date:{start_date_new.date()}|end_date:{s_end_date.date()}| Data null")
                                    continue
                                               
                                stock_factor_rq.reset_index(inplace=True)
                                stock_factor_rq['date'] = pd.to_datetime(stock_factor_rq['date'])
                                stock_factor = pd.concat([stock_factor_file, stock_factor_rq], ignore_index=True)
                    ...
                    # 保存
                    if 'index' in stock_factor.columns:
                        stock_factor.drop(columns=['index'], inplace=True) 
                    ...
                    fail_count += 1
                    logger.error(f"Stock:{code} Factor Daily Error:{str(e)}")
                finally:
                    pbar.update(1) 

        logger.success(f"Stock Factor Daily Download Completed | Success: {success_count} | Fail: {fail_count}")
        
    rqdatac.reset()
    
if __name__ == "__main__":
    fire.Fire(stock_factor_run)
