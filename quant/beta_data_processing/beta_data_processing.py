...
sys.path.append('/home/a/notebook/zxf/code')
from master.master import MASTERModel


def main(
        data_path: str = "/home/a/notebook/zxf/data/Daily_data/Beta_data/temp",
        qlib_path: str = '/home/a/notebook/cn_data_train',
    ...
    ### 1.读取配置文件
    yaml_path = f"{data_path}/workflow_config_master_Alpha158_{market_name}.yaml"
    ...
    if market_name in ["csi800c", "csi800b"]:
        mn = "csi800"
    else:
        mn = market_name
    param_path = f'{data_path}/{mn}_backday_{backday}_self_exp_{seed}_{step}.pkl'
    ...
    ### 5. 生成并保存d_model特征值数据
    # train
    df_train = model.encode(dl_train, d_model)
    ...
    # valid
    df_valid = model.encode(dl_valid, d_model)
    ...
    # test
    df_test = model.encode(dl_test, d_model)
    ...
    # 拼接adjclose
    QLIB_DIRNAME = '/home/a/notebook/qlib'
    ...
    features_filename = f"{data_path}/{market_name}_merge_exp_data_{seed}_{step}_encode_{d_model}.parquet"
