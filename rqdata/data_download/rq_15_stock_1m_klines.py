...
def get_stock_klines_file(stock_klines_filename):
    ...
    stock_klines_file['datetime'] = pd.to_datetime(stock_klines_file['datetime'])
...
def stock_klines_1m_run(
    ...
    # stock_list = stock_list[stock_list['order_book_id']=='000003.XSHE']
    ...
    if code is None:        
        ...
        with tqdm(total=len(stock_list), desc="Stock Klines 1 Minute Downloading", ncols=100) as pbar:
            ...
            for _, row in stock_list.iterrows():
                try:
                    ...
                    ### 读取本地文件
                    stock_klines_filename = f"{data_path}/basic_data/stock_klines_1m/{code}.csv"
                    try:
                        ...
                        if stock_klines_file.empty:
                            ...
                            if stock_klines_file['datetime'].iloc[-1] < s_end_date:
                                start_date_new = stock_klines_file['datetime'].iloc[-1] + pd.Timedelta(days=1)
                                ...
                                stock_klines_rq['datetime'] = pd.to_datetime(stock_klines_rq['datetime'])
                    ...
                    stock_klines['datetime'] = pd.to_datetime(stock_klines['datetime'])
                    stock_klines.to_csv(stock_klines_filename, index=False)   # , date_format='%Y-%m-%d %H:%M:%S'
                    success_count += 1    
                except Exception as e:
                    fail_count += 1
                    logger.error(f"Stock:{code} Klines 1 Minute Error:{str(e)}")
                finally:
                    pbar.update(1)            
