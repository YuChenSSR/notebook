...
### 1. 加工特征数据
def processing_predictions(workflow_path,Data_path,expt_path):
    ...
    ### 2. 读取因子
    save_path = f'{expt_path}/Predictions'
    os.makedirs(save_path, exist_ok=True)

    factor_path = f'{expt_path}/Master_results'
    ...
    for filename in factor_folder_path:
        _, _, _, _, _, seed, d_model, lr, dropout, step = filename.split('.pkl')[0].split('_')
        seed, d_model, step = int(seed), int(d_model), int(step)
        lr, dropout = float(lr), float(dropout)
        print(f"Filename:{filename} | Seed:{seed} - D_model:{d_model} - LR:{lr} - Dropout:{dropout} - Step:{step}")
        ...
        pred_frame.to_csv(f'{save_path}/master_predictions_backday_{backday}_{universe}_{seed}_{d_model}_{lr}_{dropout}_{step}.csv', index=False, date_format='%Y-%m-%d')

    test_process_info.to_csv(f'{save_path}/test_metrics_results.csv', index=False)

def main(
        data_path: str = "/home/idc2/notebook/zxf/data/modoel_training",
        market_name: str = "csi800",
        folder_name: str = "csi800_20251210_20150101_20251208",
):
    expt_path = f"{data_path}/{folder_name}"
    workflow_path = f"{expt_path}/workflow_config_master_Alpha158_{market_name}.yaml"

    processing_predictions(workflow_path=workflow_path, Data_path=data_path,expt_path=expt_path)
