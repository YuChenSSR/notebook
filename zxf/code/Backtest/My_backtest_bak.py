...
dirname = "/home/idc2/notobook/zxf/code"
...
class Backtest:
    def __init__(
            ...
            account: float = 200_000,  # 100_000_000,                       # 初始资金总额
            ...
            take_profit_ratio: float = 0.5,                                 # 止盈比例
            printing_info: bool = True,                                     # 是否打印过程明细
        ...
        self._account_details = pd.DataFrame({
            ...
            'total_assets': [],                     # 总价值
            'overall_yield': [],                    # 预估总收益（扣除成本）
            'annual_return': [],                    # 年化收益（扣除成本）
            'benchmark_return': [],                 # 基准收益率
            'cumulative_benchmark_return': [],      # 基准收益率
    ...
    def update_position(
        ...
        account_details_today['overall_yield'] = account_details_today['total_assets'] / self._account - 1
        ...
        account_details_today['annual_return'] = account_details_today['overall_yield'] * (365 / date_num.days)
    ...
    def run(self):
        ...
        # 保存最终结果
        self._position_details.to_csv(f'./position_details.csv', index=False, date_format='%Y-%m-%d')
        self._account_details.to_csv(f'./account_details.csv', index=False, date_format='%Y-%m-%d')

        print(self.evaluation())

    def evaluation(self, rf=0.000118):
        """
        rf_annual = 0.03(年化)
        rf_daily = (1 + rf_annual) ** (1/252) - 1 = 0.000118
        :param rf:
        :return:
        """
        # self._position_details
        df = self._account_details.copy()
        ...
        # 收益波动率（年化）
        volatility = df['ret'].std() * np.sqrt(annual_factor)

        # 夏普率
        sharpe = df['ret_rf'].mean() / df['ret_rf'].std() * np.sqrt(annual_factor)

        # Alpha & Beta（CAPM）
        cov = np.cov(df['ret'], df['benchmark_return'])[0][1]
        var_bench = np.var(df['benchmark_return'])
        beta = cov / var_bench
        alpha = (df['ret'].mean() - beta * df['benchmark_return'].mean()) * annual_factor

        # 信息比率 IR
        ir = df['excess_ret'].mean() / df['excess_ret'].std() * np.sqrt(annual_factor)
        ...
        annual_turnover = df['turnover'].mean() * annual_factor

        metrics = {
            "annual_return": (1 + df['ret']).prod() ** (annual_factor / len(df)) - 1,
...
def main():
    backtest = Backtest(
        ...
        is_set_loss_profit=True,
        stop_loss_ratio=-0.1,
        take_profit_ratio=1,
        market_name="csi800",

        # pred_filename=f"C:\data_set\master_run_results\predictions_data_backtest\master_predictions_backday_8_csi800c_167_19.csv",
        # pred_filename="/home/idc2/notebook/zxf/data/beta_data/csi800c_20251201_20150101_20251128/merged_predictions_dedup_minimal.csv",
        pred_filename="/home/idc2/notebook/zxf/data/master_results/master_20251204_csi800_test_data_2/Backtest_Results/predictions/master_predictions_backday_8_csi800_6041_37.csv",
