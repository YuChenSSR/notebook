...
if [ "$universe" = "csi800" ]; then
    config_file="workflow_config_master_Alpha158_4_last_day_predictions_csi800_volume.yaml"
...
# 执行数据划分
if [ "$universe" = "csi800" ]; then
    python3 data_generator_4_last_day_predictions_volume.py --universe csi800
elif [ "$universe" = "csi1000" ]; then
...
# 执行预测
if [ "$universe" = "csi800" ]; then
    python3 predict_and_send_result_volume.py csi800
elif [ "$universe" = "csi1000" ]; then
    python3 predict_and_send_result.py csi1000
else
    # 默认配置或其他股票池配置
    python3 predict_and_send_result.py other
fi
